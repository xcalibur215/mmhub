...
THAILAND_LOCATIONS = [
    "Bangkok", "Sukhumvit", "Silom", "Sathorn", "Asoke", "Phrom Phong", "Thong Lo", "Ekkamai",
    "Ari", "Saphan Phut", "Chatuchak", "Lat Phrao", "Huai Khwang", "Din Daeng", "Ratchathewi",
    "Pathum Wan", "Bang Rak", "Yannawa", "Khlong Toei", "Watthana", "Vadhana", "Dusit",
    "Phra Nakhon", "Samphanthawong", "Bang Kho Laem", "Yan Nawa", "Bang Sue", "Phaya Thai",
    "Ratchathewi", "Huai Khwang", "Din Daeng", "Bueng Kum", "Saphan Phut", "Pom Prap Sattru Phai",
    "Nong Chok", "Bang Khae", "Lak Si", "Sai Mai", "Khan Na Yao", "Saphan Sung", "Wang Thonglang",
    "Bang Kapi", "Pravej", "Lat Krabang", "Min Buri", "Nong Chok", "Chiang Mai", "Phuket", "Pattaya"
]
...
class DatabaseSetup:
    ...
    def create_amenities(self):
        ...
        if created:
            self.session.commit()
        print(f"✓ Amenities present: {len(existing) + created} (added {created}, skipped {len(existing)})")
    ...
    def create_properties(self):
        ...
        if existing_count > 0:
            print(f"✓ Properties already exist ({existing_count}); skipping property generation")
            return
        
        # Get landlords and agents to assign as property owners
        owners = self.session.query(User).filter(
            User.role.in_([UserRole.LANDLORD, UserRole.AGENT])
        ).all()
        
        # Get all amenities
        amenities = self.session.query(Amenity).all()
        
        property_types = [PropertyType.APARTMENT, PropertyType.CONDO, PropertyType.HOUSE, 
                         PropertyType.TOWNHOUSE, PropertyType.STUDIO, PropertyType.ROOM]
        ...
        for i in range(50):
            ...
            description = description_template.format(property_type.value.title(), location)
            ...
            property_data = {
                ...
                "floor_number": random.randint(1, 30) if property_type in [PropertyType.APARTMENT, PropertyType.CONDO] else None,
                ...
                "available_from": datetime.utcnow() + timedelta(days=random.randint(0, 60)),
            ...
            selected_photos = random.sample(photo_urls, min(num_photos, len(photo_urls)))
