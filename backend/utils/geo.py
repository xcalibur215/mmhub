...
def add_distance_filter(
    query, model_class, latitude: float, longitude: float, radius_km: float
):
    """
    ...
    return query.filter(
        ST_DWithin(func.geography(model_class.location), search_point, radius_meters)
    )
...
def add_distance_calculation(query, model_class, latitude: float, longitude: float):
    ...
    distance_calc = ST_Distance(
        func.geography(model_class.location), search_point
