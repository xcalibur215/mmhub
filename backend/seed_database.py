...
class DatabaseSeeder:
    def __init__(self):
        self.fake_names = [
            ...
            "Jessica Miller", "Christopher Anderson", "Amanda Taylor", "Matthew Thomas", "Ashley Jackson",
            "Daniel White", "Jennifer Harris", "Anthony Martin", "Elizabeth Thompson", "Mark Garcia",
            "Linda Rodriguez", "Steven Lewis", "Susan Walker", "Paul Hall", "Nancy Allen",
            "Kenneth Young", "Betty King", "Joshua Wright", "Helen Lopez", "Ryan Hill",
            "Michelle Green", "Brian Adams", "Sharon Nelson", "Edward Baker", "Carol Gonzalez",
            "Jason Turner", "Lisa Phillips", "Kevin Campbell", "Donna Parker", "Gary Evans",
            "Maria Edwards", "Timothy Collins", "Sandra Stewart", "Jose Sanchez", "Ruth Morris",
            "Frank Rogers", "Anna Reed", "Scott Cook", "Deborah Bailey", "Raymond Rivera",
            "Laura Cooper", "Gregory Richardson", "Janet Cox", "Alexander Howard", "Catherine Ward"
        ]
        
        self.property_titles = [
            "Luxury Downtown Apartment", "Cozy Studio Near Campus", "Modern 2BR with Balcony",
            "Spacious Family Home", "Charming Garden Apartment", "High-Rise City View",
            "Renovated Historic Loft", "Penthouse Suite", "Waterfront Condo", "Suburban Villa",
            "Minimalist Studio", "Executive Townhouse", "Artists' Loft Space", "Rooftop Terrace Apartment",
            "Classic Brownstone", "Contemporary High-Rise", "Garden Level Unit", "Corner Penthouse",
            "Converted Warehouse Loft", "Boutique Studio", "Mid-Century Modern Home", "Industrial Chic Apartment",
            "Elegant Victorian House", "Urban Retreat", "Skyline View Condo", "Riverside Apartment",
            "Designer Furnished Studio", "Luxury High-Rise Unit", "Vintage Charm Apartment", "Modern Townhouse"
        ]
        
        self.locations = [
            "Downtown Manhattan, New York", "SoHo, New York", "Brooklyn Heights, New York",
            "Beverly Hills, Los Angeles", "Santa Monica, Los Angeles", "Hollywood, Los Angeles",
            "Lincoln Park, Chicago", "River North, Chicago", "Wicker Park, Chicago",
            "South Beach, Miami", "Brickell, Miami", "Coral Gables, Miami",
            "Capitol Hill, Seattle", "Belltown, Seattle", "Queen Anne, Seattle",
            "Mission District, San Francisco", "SOMA, San Francisco", "Pacific Heights, San Francisco",
            "Back Bay, Boston", "North End, Boston", "Cambridge, Boston",
            "Dupont Circle, Washington DC", "Georgetown, Washington DC", "Adams Morgan, Washington DC"
        ]
        
        self.amenities = [
            "Pool", "Gym", "Parking", "Balcony", "Air Conditioning", "Heating",
            "Washing Machine", "Dryer", "Closet", "Granite Countertops",
            "Stainless Steel Appliances", "In-unit Laundry",
            "Doorman", "Elevator", "Pet Friendly", "Furnished", "Fireplace", "High Ceilings",
            "Floor-to-ceiling Windows", "City View", "Garden View", "Waterfront View",
            "WiFi", "Working desk"
        ]
    ...
    def create_properties(self, session: Session, landlords: List[User], agents: List[User]) -> List[Property]:
        ...
        for i in range(100):  # Create 100 properties
            ...
            monthly_rent = int(base_price * location_multiplier * (1 + random.uniform(-0.3, 0.5)))
            
            property_obj = Property(
                title=title,
                description=self.generate_property_description(title, property_type, location),
    ...
    def generate_property_description(self, title: str, property_type: str, location: str) -> str:
    ...
    def create_messages(self, session: Session, users: List[User], properties: List[Property]):
        ...
        for _ in range(random.randint(50, 80)):
            ...
            property_obj = random.choice(properties) if random.choice([True, False]) else None
